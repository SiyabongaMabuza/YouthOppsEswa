package edu.skidmore.cs276.project.rdb;
...
public class RdbSingletonFacade {
	...
	/**
	 * Set the RDB driver class name. This method attempts to load the class using
	 * a Class.forName() call. If the class cannot be found an exception will be
	 * thrown.
	 * 
	...
	public void setRdbDriverClassName(String rdbDriverClassName) {
		...
		try {
			...
			throw new IllegalArgumentException("Database driver class not found: "
					+ rdbDriverClassName, cnfExc);
		}
	}
	...
	/**
	 * Execute a DDL or DML SQL statement that does not return a result set. This
	 * includes any DDL as well as DML such as INSERT, UPDATE, and DELETE
	 * statements.
	 * 
	 * @param sql
	 *          The SQL statement
	...
	public synchronized boolean execute(String sql) {
		...
		try {
			...
			throw new RuntimeException("Unable to execute SQL statement: " + sql,
					throwable);
		} finally {
	 ...
	 * cleanUp(ResultSet) method after processing the returned ResultSet instance.
	 * 
	 * @param sql
	 ...
	 */
	public synchronized ResultSet executQuery(String sql) {
		Connection connection = null;
		Statement statement = null;
	...
	/**
	 * Validate that the required configuration information has been set. This is
	 * used prior to attempting to connect to the database.
	 */
	private void validateConfiguration() {
		...
		if (hasOpenResultSet) {
			errors
					.append("There is an active result set. "
	...
	private void readDatabase() {
		try {
			...
			Connection conn = DriverManager.getConnection(
					"jdbc:mysql://localhost:3306/tasklist", "root", "rdkf0mh");
	 ...
	 * 
	 * @param args Arguments to the method, not used
	 * 
	 * @see #readDatabase()
	 */
