...
public class JobsServlet extends HttpServlet {
	...
	private void controller(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<String[]> jobs = OpportunitiesPersistence.getInstance()
			.loadPriorOpportunities(userId, "jobs");
		LOGGER.info("Fetched jobs for user " + userId + ": " + jobs.size() + " items");
		req.setAttribute("jobs", jobs); // Pass to JSP
		forwardingPath = "/WEB-INF/hiddenjsp/jobs.jsp";
