 ...
 * @version 01.00.00
 */
public class ScholarshipsServlet extends HttpServlet {
	/**
	...
	private void controller(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<String[]> scholarships = OpportunitiesPersistence.getInstance()
			.loadPriorOpportunities(userId, "scholarships");
		LOGGER.info("Fetched scholarships for user " + userId + ": " + scholarships.size() + " items");
		req.setAttribute("scholarships", scholarships); // Pass to JSP
		forwardingPath = "/WEB-INF/hiddenjsp/scholarships.jsp";
